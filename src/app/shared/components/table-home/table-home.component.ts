...
@Component({
    ...
    <div class="w-full max-w-[900px] overflow-x-auto bg-white shadow-lg rounded-lg">
      <table class="w-full border-collapse">
        <thead>
          <tr class="bg-red-700">
            <th class="font-sans text-white font-bold text-sm p-4 text-left border-b-2 border-white">Tema</th>
            <th *ngFor="let partido of data.partidos" 
                class="font-sans text-white font-bold text-sm p-4 text-center border-b-2 border-white min-w-[150px]">
              {{ partido }}
            </th>
          </tr>
        </thead>
