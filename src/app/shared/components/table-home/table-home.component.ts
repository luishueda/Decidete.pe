...
@Component({
  ...
  template: `
    <div class="w-full max-w-[900px] overflow-x-auto bg-white shadow-lg rounded-lg">
      <table class="w-full border-collapse max-md:min-w-[600px]">
        <thead>
          <tr class="bg-red-700">
            ...
            <th *ngFor="let partido of data.partidos" 
                class="font-sans text-white font-bold text-sm p-4 text-center border-b-2 border-white min-w-[150px] max-md:p-2 max-md:text-xs max-md:min-w-[100px]">
              {{ partido }}
            </th>
          </tr>
        </thead>
